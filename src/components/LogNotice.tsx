 ...
 *     反复点过名的那种错(屏幕上什么都不说)。
 *
 * ## ⚠️ 那句话里**不许出现「这张照片」**(2026-09-24 晚改口)
 *
 * 原来两句都写着「正在**拿这张照片**算营养」。补记那份草稿还能来自打字
 * (`UnloggedMeal.from === 'text'`,见 `store/unlogged.ts` 文件头)—— 用户问一句
 * 「红烧肉怎么做」,点完「记入日记」看到的就是这张卡,而他**一张照片都没有**。
 * 用户那天点着这句话问:「为什么弹出来的窗口写的是『正在拿这张照片算营养』」。
 *
 * 于是两句都收成**不指图**的说法:这张卡的任务只有「按下去之后会发生什么」,
 * 「重新算一遍」这件事是照片那条路的细节,不是这里要说的。两个来源共用一句话 ——
 * 这张卡**够不着**草稿的 `from`(它只拿到 `noticeFor` 那三个字段),而它说的
 * 那句对两条路都成立。
 *
 * ⚠️ 于是它也**不是**「通用到底」的模板:它仍然说清了「算好会自动记进日记」
 * 还是「打开记录面板」—— 那是两条出口的差别,不能一起抹掉(见下面 `kind`)。
 *
 * ## 为什么 `kind` 是一个 prop
 *
      ...
      <div className="animate-fade-in absolute top-1/2 left-1/2 flex w-[78%] -translate-x-1/2 -translate-y-1/2 flex-col gap-4 rounded-[20px] bg-[#FAFAFC] px-5 py-5">
        {error ? (
          ...
          <p className="text-[15px] leading-[22px] text-ink">
            {kind === 'adjust'
              ? '正在算这一餐的营养，算好了打开记录面板让你改分量。'
              : done
                ? '已经记进日记了。'
                : '正在算这一餐的营养，算好了会自动记进日记。'}
          </p>
        )}
