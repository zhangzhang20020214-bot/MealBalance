        ...
        <div className="flex shrink-0 flex-col gap-1 px-1 pt-3 pb-3">
          ...
          */}
          <span className="text-[13px] leading-[19.5px] text-muted">
            ...
            */}
            {/*
              「识别到的」/「聊到的」按草稿是哪条路来的分（2026-09-24）：
              打字那份一张照片都没有，说「识别到的」是在陈述一件没发生过的事。
              中间那个空格两边都要留（理由见上面那段）。
            */}
            {unloggedWhen(meal.at)} {meal.from === 'text' ? '聊到的' : '识别到的'}这几道菜，还没记进日记。
          </span>
        </div>
        ...
        <div className="flex shrink-0 flex-col gap-2">
          ...
          */}
          ...
          {/*
            ...
            ⚠️ 2026-09-24 修过一次口误：打字那份原来写的是「按食物库的常见分量
            算一遍，**不用等**」—— 那是本地凑数那一版的遗留（营养不该在本地算，
            见 `store/unlogged.ts` 文件头）。现在两条路都交给食衡，而食衡那一趟
            实测 13~26 秒（库外菜要联网搜），所以**两条都要说「要等十几秒」**。
            说「不用等」的代价是用户点完盯着不动的屏幕怀疑它死了。
          */}
          <p className="px-1 text-[12px] leading-[17.38px] text-faint">
            {meal.from === 'text'
              ? '记进去之前会拿这几道菜问一遍食衡算营养，要等十几秒。算出来的分量不是你选的 —— 想改就点上面的「调整分量再记」。'
