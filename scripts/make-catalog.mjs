...
/* ------------------------------------------------------------
   ...
   · **「也不要挑一个『最像的』顶上去」**(2026-09-24 补)—— 光说「编一个」不够。
     实测(qwen3.7-plus)模型不编了,改成挑目录里最接近的那条:「土豆炒肉丝」
     挑中「青椒肉丝」的 id,因为它俩都是肉丝炒菜。后果比编一个更重 ——
     编的 id 校验不过、退回名字匹配,**错得能被发现**;挑一个真实存在的 id
     则一路通行,那道菜从此顶着**另一道菜的营养**。而三张图一起读之后,
     它还会和真正的青椒肉丝**并成一行**(`mergeMeals` 按 foodId 合并),
     屏幕上少一道菜、克数取两者最大。所以反例必须写进提示词,不能只写「别编」
...
3. 这道菜在目录里没有对应项时,**不要输出 foodId 这个字段** —— 不要为了填满
   ...
   **也不要挑一个「最像的」顶上去**:目录里有青椒肉丝,不等于「土豆炒肉丝」
   可以用青椒肉丝的 id —— 那是两道菜,挑错了会按青椒肉丝算它的营养。
   目录里挑不到,就**不填**
